...
class MultilayerPerceptron:
    ...
    def init_weights(self):
        ...
        weights = np.random.randn(
            self.hidden_layer_amount[0], self.n_inputs + 1)
    ...
    def feedforward(self, X):
        ...
        for i in np.arange(len(self.hidden_layer_amount)):
            activation = self.sigmoid_activation(
                np.dot(self.activations[i], self.weights_per_layer[i].T))
            self.activations.append(np.insert(activation, 0, 1))

        # Compute the activations of the output layer
        activation = self.sigmoid_activation(
            np.dot(self.activations[-1], self.weights_per_layer[-1].T))
    ...
    def predictfeedforward(self, X):
        if X.ndim == 1:
            ...
            for i in np.arange(len(self.hidden_layer_amount)):
                current_layer_activation = self.sigmoid_activation(
                    np.dot(current_layer_activation, self.weights_per_layer[i].T))
                current_layer_activation = np.insert(
                    current_layer_activation, 0, 1)
            current_layer_activation = self.sigmoid_activation(
                np.dot(current_layer_activation, self.weights_per_layer[-1].T))
        else:
            current_layer_activation = np.insert(X, 0, 1, axis=1)

            for data_activation in current_layer_activation:
                for i in np.arange(len(self.hidden_layer_amount)):
                    data_activation = self.sigmoid_activation(
                        np.dot(data_activation, self.weights_per_layer[i].T))
                    data_activation = np.insert(data_activation, 0, 1)
                data_activation = self.sigmoid_activation(
                    np.dot(data_activation, self.weights_per_layer[-1].T))
    ...
    def backpropagation(self, X, Y, learning_rate=0.1):
        ...
        for i in np.arange(len(self.hidden_layer_amount), 0, -1):
            ...
            output_error = np.dot(
                self.weights_per_layer[i].T, last_layer_delta)
            # print(output_error)
            # print(self.sigmoid_activation_derivative(self.activations[i]))
            layer_delta = np.dot(np.array(
                output_error).T, self.sigmoid_activation_derivative(self.activations[i]))
        ...
        self.weights_per_layer[-1] += learning_rate * \
            np.dot(last_layer_delta, self.activations[-2])

        # Update the weights of the hidden layers
        for i in np.arange(len(self.hidden_layer_amount), 0, -1):
            delta_w = learning_rate * \
                np.dot(layer_deltas[i], self.activations[i-1])
            self.weights_per_layer[i-1] += delta_w
            if self.momentum is not None:
                self.weights_per_layer[i-1] += self.momentum * \
                    self.prev_weights_per_layer[i-1]
