...
class MultilayerPerceptron:
    def __init__(self, layer_sizes, momentum=None):
        ...
        self.weights = [np.random.randn(
            self.layer_sizes[i], self.layer_sizes[i+1]) for i in range(self.num_layers - 1)]
        ...
        if self.momentum is not None:
            ...
            self.prev_weights = [np.zeros(
                (self.layer_sizes[i], self.layer_sizes[i+1])) for i in range(self.num_layers - 1)]
            self.prev_biases = [np.zeros((1, self.layer_sizes[i+1]))
                                for i in range(self.num_layers - 1)]
    ...
    def feedforward(self, X):
        ...
        for i in range(self.num_layers - 1):
            self.outputs.append(
                np.dot(self.activations[i], self.weights[i]) + self.biases[i])
        ...
        for i in range(self.num_layers - 1):
            if self.momentum is not None:
                delta_w = learning_rate * \
                    ...
                    self.momentum * self.prev_weights[i]
                delta_b = learning_rate * \
                    np.sum(deltas[i], axis=0, keepdims=True) + \
                    self.momentum * self.prev_biases[i]
